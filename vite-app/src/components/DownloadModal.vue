    ...
    <div class="modal-box">
      <div class="modal-header-grad">
        <div class="modal-title">
          ...
          <p>Your data is being prepared and will download shortly.</p>
        </div>
        <i class="fa-solid fa-file-csv" style="font-size:2.5rem;color:#aaddff;opacity:0.8;"></i>
      </div>
      <div class="modal-body">
        ...
        <div v-if="activeTab === 'gis'">
          <p style="font-size:0.9rem;margin-bottom:10px;"><i class="fa-solid fa-triangle-exclamation" style="color:#f39c12;"></i> To visualize this data, load the reference geometry service below, then join your downloaded CSV.</p>
          ...
          <ul style="font-size:0.9rem;color:#444;line-height:1.6;padding-left:18px;">
...
const pyScript = `# Install h3 and dependencies
...
# 1. Read Data
df = pd.read_csv(folder_path / "HousingSiteEvaluator_Filtered_Data.csv")
...
# 4. Save as GeoJSON
gdf.to_file(folder_path / "HousingSiteEvaluator_Filtered_Data.geojson", driver="GeoJSON")`

async function applyHighlight() {
  await nextTick()
  if (typeof window.hljs !== 'undefined') {
    document.querySelectorAll('pre code:not([data-highlighted])').forEach(el => window.hljs.highlightElement(el))
  }
}

onMounted(applyHighlight)
watch(activeTab, applyHighlight)
</script>
